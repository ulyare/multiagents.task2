...
public class main2 extends Agent {
    ...
    static class Config {
        ...
        static final double ALPHA = 0.2; // Коэффициент доверия alpha из (0, 1]
        ...
        static {
            ...
            double[] valueData = {35, -7, 5, 34, 7, 30, 11, -15, 42, -23, 18, 29, -8, 51, 47};
    ...
    class SendBehaviour extends TickerBehaviour {
        ...
        // Выбор случайных ребер для отбрасывания (0, 1 или 2 с вероятностью ~33% каждая)
        ...
        private List<Integer> getActiveNeighbors() {
            List<int[]> currentDropped = new ArrayList<>(Config.droppedEdges);
            List<Integer> active = new ArrayList<>();
            for (int n : Config.getNeighbors(data.id)) {
                boolean isDropped = false;
                for (int[] edge : currentDropped) {
        ...
        private void sendToNeighbors(List<Integer> neighbors) {
            ...
            try {
                msg.setContentObject(new NodeData(data.id, data.value));
        ...
        private void sendStatus() {
            ...
            statusMsg.setContent(stage + "," + data.id + "," + data.value);
            send(statusMsg);
        }
    }
        ...
        public void action() {
            ...
            if (msg != null) {
                try {
                    ...
                    double noisyValue = otherData.value + (Math.random() - 0.5); // Добавление шума uniform[-0.5, 0.5]
                    received.add(noisyValue);

                    // Обновление значения по протоколу локального голосования, когда получены все ожидаемые сообщения
                    // values[i] += alpha * sum_{j in N_i} (values[j] - values[i])
                    if (received.size() == getActiveNeighbors().size()) {
                        double sum_diff = 0.0;
                        for (double v : received) {
                            sum_diff += (v - data.value);
                        }
                        data.value += Config.ALPHA * sum_diff;
                        received.clear();
                    }
                } catch (Exception e) {
    ...
    class StopBehaviour extends Behaviour {
        ...
        @Override
        public boolean done() { return stopFlag; }
    }
            ...
            if (msg != null) {
                ...
                if (msgStage == currentStage) {
                    ...
                    if (receivedValues.size() == Config.TOTAL_AGENTS) {
                        ...
                        if (currentStage == Config.MAX_STAGES) {
                            calculateRealAverage(); // Расчет только после последнего шага
                        }
                        currentStage++;
                    }
                }
        ...
        private void printStageResults() {
            ...
            if (Config.droppedEdges.isEmpty()) System.out.println("-");
                ...
                for (int i = 0; i < Config.droppedEdges.size(); i++) {
                    ...
                    if (edge != null) { // Проверка на null для безопасности
